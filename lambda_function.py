...
def lambda_handler(event, context):
    ...
    while image_found == False:
        ...
        if random_post.url.endswith('.jpg') or random_post.url.endswith('.png'):
            urllib.request.urlretrieve(random_post.url, "/tmp/random_image.jpg")
            image_found = True

    me_me_image = Path(__file__).parent.joinpath("/tmp/random_image.jpg").read_bytes()
    ...
    #mail attachment
    with open("/tmp/random_image.jpg", 'rb') as file:
        img_data = file.read()
        image = MIMEImage(img_data, name="/tmp/random_image.jpg")
    ...
    os.remove("/tmp/random_image.jpg")
